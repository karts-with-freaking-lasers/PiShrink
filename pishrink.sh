...
if [[ $prep == true ]]; then
  ...
  mountdir=$(mktemp -d)
  mount "$loopback" "$mountdir"
  rm -rvf $mountdir/var/cache/apt/archives/* $mountdir/var/lib/dhcpcd5/* $mountdir/var/log/!(mosquitto) $mountdir/var/tmp/* $mountdir/tmp/* #[edit]keep ssh keys$mountdir/etc/ssh/*_host_*
  umount "$mountdir"
fi
